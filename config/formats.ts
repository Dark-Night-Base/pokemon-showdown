...
export const Formats: import('../sim/dex-formats').FormatList = [
	...
	},
	{
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if ((set as any).fusionSpecies) set.name = set.species = (set as any).fusionSpecies.name;
			}
		},
		// keep special fusion movepool
		checkCanLearn(move, species, lsetData, set) {
			if ((set as any).fusionSpecies) {
				const problem = this.checkCanLearn(move, (set as any).fusionSpecies);
		...
		validateSet(set, teamHas) {
			...
			if (fusionSpecies.species) {
				...
				(set as any).fusionSpecies = fusionSpecies.species;
			} else {
		...
		onBegin() {
			for (const pokemon of this.getAllPokemon()) {
				...
				// moved to Pokemon::getUpdatedDetails()
	...
	},
	{
		...
		banlist: [
			...
			'Chi-Yu', 'Chien-Pao', 'Darkrai', 'Deoxys', 'Dialga', 'Dracovish', 'Dragapult', 'Espathra',
			'Eternatus', 'Flutter Mane', 'Genesect', 'Giratina', 'Gouging Fire', 'Groudon', 'Ho-Oh', 'Iron Bundle',
			'Kangaskhan-Mega', 'Koraidon', 'Kyogre', 'Landorus', 'Lucario-Mega', 'Lugia', 'Lunala', 'Magearna',
			'Marshadow', 'Metagross-Mega', 'Mewtwo', 'Miraidon', 'Naganadel', 'Necrozma-Ultra', 'Palafin', 'Palkia',
			'Pheromosa', 'Rayquaza', 'Reshiram', 'Salamence-Mega', 'Shaymin-Sky', 'Sneasler', 'Solgaleo', 'Spectrier',
			'Terapagos-Stellar', 'Urshifu', 'Walking Wake', 'Xerneas', 'Yveltal', 'Zacian', 'Zamazenta-Crowned', 'Zekrom',
			'Zygarde',
			'Cramorant', 'Shedinja',
	...
	},
	{
		...
		ruleset: [
			// position matters for 'NatDex Mod'
			'Obtainable', 'NatDex Mod', '+Unreleased', 'Team Species Preview', 'Nickname Preview', '!!EV Limit = 1020', 'Max Level = 5', 'Species Clause',
			'HP Percentage Mod', 'Cancel Mod', 'Endless Battle Clause', 'Dynamax Clause', 'Terastal Clause',
			'OHKO Clause', 'Evasion Moves Clause', 'Sleep Moves Clause',
		],
		banlist: [
		...
		validateSet(set, teamHas) {
			const headSpecies = this.dex.species.get(set.name);
			if (headSpecies.exists) { // Little Cup check
			...
			const bodySpecies = this.dex.species.get(set.species);
			if (bodySpecies.exists) { // Little Cup check
	...
	{
		name: "[Gen 9] Infinite Fusion Monotype",
		desc: `将（几乎）任意两只精灵融合成新精灵！<br/>\
			<details><summary>具体规则（点击展开）</summary>\
			&bullet; 正常选择精灵作为融合的<strong>身体</strong>部分，在昵称栏写上一只精灵的英文名，由它组成<strong>头部</strong><br/>\
			&bullet; 融合后精灵的ＨＰ、特攻、特防种族 = 头部对应种族的 2 / 3 + 身体对应种族的 1 / 3<br/>\
		...
		onValidateTeam(team) {
			...
			for (const [i, set] of team.entries()) {
				// species clause
				let name = this.dex.species.get(set.name);
				let species = this.dex.species.get(set.species);
				if (name.exists && names.has(name.id)) {
					return [
						`You have more than one ${name}`,
					];
				}
				if (names.has(species.id)) {
					return [
						`You have more than one ${species}`,
					];
				}
				if (name.exists) names.add(name.id);
				if (species.id !== name.id) names.add(species.id);

				// Nihilslave: if the pokemon is a special fusion, change it here
				if ((set as any).fusionSpecies) set.name = set.species = (set as any).fusionSpecies.name;

				// same type clause
				name = this.dex.species.get(set.name);
				species = this.dex.species.get(set.species);
				let types = [];
				if (!name.exists) {
					types = species.types;
				} else {
					types.push(name.types[0]);
					if (species.types.length > 1) types.push(species.types[1]);
					else types.push(species.types[0]);
				}
				if (i === 0) typeTable = types;
				else typeTable = typeTable.filter(type => types.includes(type));
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			const originalFormeSecies = this.dex.species.get((pokemon.species as any).originalSpecies);
	...
	},
	{
		...
		validateSet(set, teamHas) {
			const validatePlugin = function (this: TeamValidator, type: 'item' | 'ability') {
				...
				if (set.moves.map(this.toID).includes(move.id) && move.id !== '')
					return [`${set.name} cannot have move ${move.name} for more than once`];
				if (move.isNonstandard && ["CAP", "LGPE", "Custom", "Gigantamax"].includes(move.isNonstandard))
					return [`${move.name} does not exist in the game`];
				const bannedRestrictedMoves = ['assist', 'entrainment', 'skillswap'];
				if (this.ruleTable.isRestricted(`move:${move.id}`) || bannedRestrictedMoves.includes(move.id))
				...
				(move.boosts?.accuracy && move.boosts.accuracy < 0);
				const evasionRaisingMove = move.boosts?.evasion && move.boosts.evasion > 0;
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			set.moves = set.moves.filter(move =>
				this.dex.moves.get(this.toID(move)).exists && this.toID(move) !== 'metronome' as ID);
			if (!this.dex.items.get(item).exists) set.item = '';
			if (!this.dex.abilities.get(ability).exists) set.ability = 'ballfetch';
			problems = this.validateSet(set, teamHas) || [];
	...
	},
	{
		...
		onModifyMove(move, pokemon, target) {
			...
			if (move.target === 'all') {
				...
				move.onHitField = function (_target, _pokemon, _move) {
					const result = moveOnHitField?.call(this, _target, _pokemon, _move);
					if (result === false) return result;
					if (!_pokemon.transformed) _move.willChangeForme = true;
				};
			} else if (move.target === 'foeSide' || move.target === 'allySide') {
				const moveOnHitSide = move.onHitSide;
				move.onHitSide = function (_target, _pokemon, _move) {
					const result = moveOnHitSide?.call(this, _target, _pokemon, _move);
					if (result === false) return result;
					if (!_pokemon.transformed) _move.willChangeForme = true;
				};
			} else {
				const moveOnHit = move.onHit as Function;
				move.onHit = function (_target, _pokemon, _move) {
					const result = moveOnHit?.call(this, _target, _pokemon, _move);
					if (result === false) return result;
					if (!_pokemon.transformed) _move.willChangeForme = true;
				};
			}
			const moveOnAfterMoveSecondarySelf = move.onAfterMoveSecondarySelf;
			move.onAfterMoveSecondarySelf = function (_pokemon, _target, _move) {
				moveOnAfterMoveSecondarySelf?.call(this, _pokemon, _target, _move);
				if (_move.willChangeForme) {
					if (!_pokemon.m.relic) {
						_pokemon.m.relic = _pokemon.formeChange(
							_pokemon.m.relicSpecies, this.effect, true, `${_pokemon.m.rawSpecies} transformed!`);
						this.add('-start', _pokemon, 'typechange', _pokemon.species.types.join('/'), '[silent]');
					} else {
						_pokemon.m.relic = !_pokemon.formeChange(
							_pokemon.m.rawSpecies, this.effect, true, `${_pokemon.m.rawSpecies} transformed!`);
					}
				}
			};
		},
	...
	},
	{
		...
		mod: 'gen9',
		ruleset: ['Standard OMs', 'Ability Clause = 2', 'Sleep Moves Clause', '!Obtainable Abilities', '+Unobtainable'],
		banlist: [
		...
		checkCanLearn(move, species, lsetData, set) {
			...
			const problem = convCheckCanLearn(this, move, (set as any).sp);
			if (!problem) return null;
			if (convCheckCanLearn(this, move, (set as any).crossSpecies)) return problem;
			return null;
		},
		validateSet(set, teamHas) {
			...
			// set types for validation // Nihilslave on 20250513: i don't remember what this part does but just keep
			...
			(set as any).crossSpecies = { ...crossSpecies, types: mixedTypes };
			problems = this.validateSet(set, teamHas);
		...
		onValidateSet(set, format) {
			const species = (set as any).crossSpecies || set.species;
			...
			const matchingSpecies = this.dex.species.all()
				.filter(_species => (
					(!_species.isNonstandard || this.ruleTable.has(`+pokemontag:${this.toID(_species.isNonstandard)}`)) &&
					_species.types.every(type => curSpecies.types.includes(type)) &&
					_species.types.length === curSpecies.types.length && !this.ruleTable.isBannedSpecies(_species)
				));
			for (const _species of matchingSpecies) {
				for (const abilityName of Object.values(_species.abilities)) {
	...
	},
	{
		...
		banlist: [
			...
			'Gengarite',
			'Belly Drum', 'Bolt Beak', 'Ceaseless Edge', 'Chatter', 'Double Iron Bash', 'Electrify', 'Last Respects', 'Octolock', 'Rage Fist', 'Revival Blessing',
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			const accuracyLoweringMove = move.secondaries?.some(secondary =>
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			const oMegaSpecies = this.dex.species.get((pokemon.species as any).originalMega);
			if (oMegaSpecies.exists && pokemon.m.originalSpecies !== oMegaSpecies.baseSpecies) {
				// Place volatiles on the Pokémon to show its mega-evolved condition and details
		...
		pokemon: {
			...
			ignoringAbility() {
				...
				return !!(
					...
					((this.volatiles['gastroacid'] || (neutralizinggas && this.ability !== ('neutralizinggas' as ID) &&
						this.item !== ('neutralizinggas' as ID))) && !this.getAbility().flags['cantsuppress']));
			},
			ignoringItem() {
				let nGas = false;
				for (const pokemon of this.battle.getAllActive()) {
			...
			takeItem(source) {
				...
				if (this.battle.runEvent('TakeItem', this, source, null, item)) {
					this.item = '';
					this.itemState = this.battle.initEffectState({ id: '', target: this });
	...
	},
	{
		...
		banlist: [
			'Smeargle',
			'Arena Trap', 'Huge Power', 'Moody', 'Pure Power', 'Shadow Tag', 'Stakeout', 'Water Bubble',
	...
	},
	{
		...
		mod: 'gen9',
		ruleset: ['Standard OMs', 'Sleep Moves Clause', 'Alphabet Cup Move Legality'],
		banlist: [
	...
	},
	{
		...
		mod: 'mirrormove',
		debug: true,
		ruleset: ['Standard OMs', '!Evasion Moves Clause', 'Evasion Clause'],
		banlist: [
	...
	},
	{
		...
		mod: 'gen9',
		ruleset: ['Hackmons Forme Legality', 'Species Reveal Clause', 'Showdown'],
	},
	{
	...
	},
	{
		...
		mod: 'gen9',
		ruleset: ['Standard OMs', 'Evasion Clause', 'Tier Shift Mod', 'Sleep Moves Clause', 'Terastal Clause'],
		banlist: [
	...
	},
	{
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (itemTable.has(item.id) && (item.megaStone || item.isPrimalOrb)) {
					return [
		...
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
			const item = this.dex.items.get(set.item);
			if (!item.megaEvolves && !item.isPrimalOrb && item.id !== 'ultranecroziumz') return;
			if (species.baseSpecies === item.megaEvolves || (item.isPrimalOrb && item.itemUser?.includes(species.baseSpecies)) ||
