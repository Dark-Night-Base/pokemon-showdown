...
export const Rulesets: {[k: string]: FormatData} = {
		...
		onValidateTeam(team) {
			...
			if (finalPoints > pointLimit) {
				...
				for (let i = 0; i < pointDetails.length; ++i) {
					...
					problems.push(`- S: ${pointDetails[i][0]} = ${pointDetails[i][1].toFixed(1)} * ${pointDetails[i][2]} / 800`);
					problems.push(`- T: ${pointDetails[i][3].toFixed(1)} = ${pointDetails[i][5] === -1 ? `${pointDetails[i][4]} * 1.5` : `${pointDetails[i][4]} + ${pointDetails[i][5]}`}`);
					problems.push(`- A: ${pointDetails[i][6]}`);
					problems.push(`- M: ${pointDetails[i][7]} = ${pointDetails[i][8]} + ${pointDetails[i][9]} + ${pointDetails[i][10]} + ${pointDetails[i][11]}`);
					if (pointDetails[i][12]) problems.push(`- P: ${pointDetails[i][12]} = 2 * ${pointDetails[i][13]} + ${pointDetails[i][14]} + ${pointDetails[i][15]} + ${pointDetails[i][16]} + ${pointDetails[i][17]} + ${pointDetails[i][18]}`);
	...
	signatureitemsclause: {
		...
		banlist: [
			...
			'Rusted Sword', 'Thick Club', 'Soul Dew',
		],
		onBegin() {
			...
			for (const side of this.sides) {
				for (const pokemon of side.pokemon) {
					...
					if (this.ruleTable.has(`nicknamepreview`)) {
						// pooooookemon should already have their headSpecies set at this point, i guess?
	...
	miamovelegality: {
		...
		restricted: [
			...
			'Echoed Voice', 'Eerie Spell', 'Fishious Rend', 'Fury Cutter', 'Ice Ball', 'Inferno', 'Jaw Lock',
			...
			'Assist', 'Baneful Bunker', 'Bestow', 'Block', 'Copycat', 'Detect', 'Destiny Bond', 'Encore', 'Fairy Lock',
			'Ingrain', 'Instruct', 'King\'s Shield', 'Mean Look', 'Obstruct', 'Octolock', 'Protect', 'Revival Blessing', 'Roar',
			'Skill Swap', 'Silk Trap', 'Spiky Shield', 'Sleep Talk', 'Shed Tail', 'Shell Smash', 'Spider Web', 'Substitute',
			'Switcheroo', 'Trick', 'Trick Room', 'Whirlwind',
		],
	},
	miaclause: {
		effectType: 'ValidatorRule',
