...
export const commands: Chat.ChatCommands = {
	mbh: 'morebalancedhackmons',
	mbhv4: 'morebalancedhackmons',
	morebalancedhackmons(target, room, user, connection, cmd) {
		if (!this.runBroadcast()) return;

		// const {dex, format, targets} = this.splitFormat(target, true);
		const dex = Dex.mod('gen9morebalancedhackmons').includeData();
		const format = Dex.formats.get('gen9morebalancedhackmons');
		...
		if (!targetId) return this.parse('/help morebalancedhackmons');
	...
	morebalancedhackmonshelp: [
		`/mbh [pokemon/item/move/ability/nature] - Get details on this pokemon/item/move/ability/nature in mbhv4.`,
		`!mbh [pokemon/item/move/ability/nature] - Show everyone these details. Requires: + % @ # &`,
	],
};
