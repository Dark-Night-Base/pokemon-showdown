...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	createmonsmod: {
		effectType: 'Rule',
		name: "Createmons Mod",
		hasValue: true, // actually 'positive-integer', changed to true to allow unlimited pointLimit
		onValidateSet(set) {
			let statName: StatID;
		...
		onValidateTeam(team) {
			const pointLimit = parseInt(this.ruleTable.valueRules.get('createmonsmod')!); // this can be NaN, which means unlimited
			let finalPoints = 0;
			const pointDetails = team.map(set => getSetPoint(this.dex, set));
			for (const details of pointDetails) {
				const Total = Math.floor(details[0] * details[4] * details[7] * details[8]);
				finalPoints += Total;
				details.push(Total);
			}
			const problems: string[] = [];
			if (finalPoints > pointLimit) {
				...
				for (let i = 0; i < pointDetails.length; ++i) {
					...
					problems.push(`- S: ${pointDetails[i][0]} = ${pointDetails[i][1].toFixed(2)} * ${pointDetails[i][2].toFixed(2)} * ${pointDetails[i][3].toFixed(2)} * 10`);
					problems.push(`- T: ${pointDetails[i][4].toFixed(1)} = ${pointDetails[i][6] === -1 ? `${pointDetails[i][5]} * 1.5` : `${pointDetails[i][5]} + ${pointDetails[i][6]}`}`);
					problems.push(`- A: ${pointDetails[i][7]}`);
					problems.push(`- M: ${pointDetails[i][8]} = ${pointDetails[i][9]} + ${pointDetails[i][10]} + ${pointDetails[i][11]} + ${pointDetails[i][12]}`);
	...
	miamovelegality: {
		...
		restricted: [
			...
			'Dynamic Punch', 'Echoed Voice', 'Eerie Spell', 'Fishious Rend', 'Fury Cutter', 'Ice Ball',
		...
		onValidateTeam(team, format) {
			...
			for (const set of team) {
				...
				if (nameSpecies.exists && nameSpecies.num !== species.num) {
					...
					// check isNonstandard
					if (nameSpecies.isNonstandard &&
						!(this.ruleTable.has(`+pokemontag:${this.toID(nameSpecies.isNonstandard)}`) ||
							this.ruleTable.has(`+pokemon:${nameSpecies.id}`) ||
							this.ruleTable.has(`+basepokemon:${this.toID(nameSpecies.baseSpecies)}`))) {
