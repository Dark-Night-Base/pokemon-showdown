 ...
 *
 * This plugin allows rooms to run games of scripted SGS
 *
 * @license MIT license
 */
...
export class SGS extends Rooms.RoomGame<SGSPlayer> {
	playerCap: number;
	...
	onConnect(user: User, connection: Connection) {
		if (this.state === 'signups') {
			connection.sendTo(
				...
				`<p style="font-size: 14pt; text-align: center">A new game of <strong>SGS</strong> is starting!</p>` +
			...
			connection.sendTo(
				this.room,
				`|uhtml|uno-${this.gameNumber}|<div class="infobox"><p>A game of SGS is currently in progress.<button class="button" name="send" value="/uno spectate">Spectate Game</button></p>` +
	...
	onStart(isAutostart?: boolean) {
		if (this.playerCount < 2) {
			if (isAutostart) {
				this.room.add("The game of SGS was forcibly ended because there aren't enough users.");
				this.destroy();
				return false;
			} else {
				throw new Chat.ErrorMessage("There must be at least 2 players to start a game of SGS.");
			}
		}
		if (this.autostartTimer) clearTimeout(this.autostartTimer);
		this.sendToRoom(`|uhtmlchange|uno-${this.gameNumber}|<div class="infobox"><p>The game of SGS has started. <button class="button" name="send" value="/uno spectate">Spectate Game</button></p>${this.suppressMessages ? `<p style="font-size: 6pt">Game messages won't show up unless you're playing or watching.` : ''}</div>`, true);
		this.state = 'play';
	...
	joinGame(user: User) {
		if (user.id in this.playerTable) {
			throw new Chat.ErrorMessage("You have already joined the game of SGS.");
		}
		if (this.state === 'signups' && this.addPlayer(user)) {
			this.sendToRoom(`${user.name} has joined the game of SGS.`);
			return true;
		}
		return false;
	}
	...
	leaveGame(user: User) {
		...
		if ((this.state === 'signups' && this.removePlayer(user)) || this.eliminate(user.id)) {
			this.sendToRoom(`${user.name} has left the game of SGS.`);
			return true;
		}
		return false;
	}
	...
	/**
	 * Overwrite the default makePlayer so it makes an SGSPlayer instead.
	 */
	makePlayer(user: User) {
		return new SGSPlayer(user, this);
	}
	...
	onPlay(player: SGSPlayer, cardName: string) {
		if (this.currentPlayerid !== player.id || this.state !== 'play') return false;
	...
	onSelectColor(player: SGSPlayer, color: Color) {
		if (
	...
	onDrawCard(player: SGSPlayer, count: number) {
	...
	onUno(player: SGSPlayer, unoId: ID) {
		// uno id makes spamming /uno uno impossible
		if (this.unoId !== unoId || player.id !== this.awaitUno) return false;
		this.sendToRoom(`|c:|${Math.floor(Date.now() / 1000)}|&|**SGS!** ${player.name} is down to their last card!`);
		this.awaitUno = null;
		this.unoId = null;
	}
	...
	onCheckUno() {
		if (this.awaitUno) {
			// if the previous player hasn't hit SGS before the next player plays something, they are forced to draw 2 cards;
			if (this.awaitUno !== this.currentPlayerid) {
				this.sendToRoom(`|c:|${Math.floor(Date.now() / 1000)}|&|${this.playerTable[this.awaitUno].name} forgot to say SGS! and is forced to draw 2 cards.`);
	...
	onWin(player: SGSPlayer) {
		this.sendToRoom(
			Utils.html`|raw|<div class="broadcast-blue">Congratulations to ${player.name} for winning the game of SGS!</div>`,
			true
		);
		this.destroy();
	}
	...
	destroy() {
		if (this.timer) clearTimeout(this.timer);
		if (this.autostartTimer) clearTimeout(this.autostartTimer);
		this.sendToRoom(`|uhtmlchange|uno-${this.gameNumber}|<div class="infobox">The game of SGS has ended.</div>`, true);
...
class SGSPlayer extends Rooms.RoomGamePlayer<SGS> {
	hand: Card[];
	cardLock: string | null;

	constructor(user: User, game: SGS) {
		super(user, game);
		this.hand = [];
		this.cardLock = null;
	}
	...
	sendDisplay() {
		...
		const uno = `<button class="button" style=" width: 90%; background: rgba(0, 255, 0, 0.05); height: 60px; margin-top: 2px;" name=send value="/uno uno ${this.game.unoId || '0'}">SGS!</button>`;
	...
	sgs: {
		...
		disable(target, room, user) {
			...
			if (room.settings.unoDisabled) {
				throw new Chat.ErrorMessage("SGS is already disabled in this room.");
			}
			room.settings.unoDisabled = true;
			room.saveSettings();
			return this.sendReply("SGS has been disabled for this room.");
		},
		...
		enable(target, room, user) {
			...
			this.checkCan('gamemanagement', null, room);
			if (!room.settings.unoDisabled) {
				throw new Chat.ErrorMessage("SGS is already enabled in this room.");
			}
			delete room.settings.unoDisabled;
			room.saveSettings();
			return this.sendReply("SGS has been enabled for this room.");
		},
		...
		create(target, room, user, connection, cmd) {
			...
			if (room.settings.unoDisabled) throw new Chat.ErrorMessage("SGS is currently disabled for this room.");
			...
			if (isNaN(cap)) cap = 12;
			if (cap < 2) cap = 2;
			room.game = new SGS(room, cap, suppressMessages);
			this.privateModAction(`A game of SGS was created by ${user.name}.`);
			this.modlog('SGS CREATE');
		},
		...
		setcap(target, room, user) {
			room = this.requireRoom();
			this.checkCan('minigame', null, room);
			const game = this.requireGame(SGS);
			if (game.state !== 'signups') {
				throw new Chat.ErrorMessage(`There is no SGS game in the signups phase in this room, so adjusting the player cap would do nothing.`);
			}
			let cap = parseInt(target);
			if (isNaN(cap) || cap < 2) {
				cap = 2;
		...
		start(target, room, user) {
			...
			const game = this.requireGame(SGS);
			if (game.state !== 'signups') {
				throw new Chat.ErrorMessage("There is no SGS game in signups phase in this room.");
			}
			game.onStart();
			this.privateModAction(`The game of SGS was started by ${user.name}.`);
			this.modlog('SGS START');
		},
		...
		stop: 'end',
			...
			if (!room.game || room.game.gameid !== 'uno') {
				throw new Chat.ErrorMessage("There is no SGS game going on in this room.");
			}
			room.game.destroy();
			room.add("The game of SGS was forcibly ended.").update();
			this.privateModAction(`The game of SGS was ended by ${user.name}.`);
			this.modlog('SGS END');
		},
		...
		timer(target, room, user) {
			...
			this.addModAction(`${user.name} has set the SGS automatic disqualification timer to ${amount} seconds.`);
			this.modlog('SGS TIMER', null, `${amount} seconds`);
		},
		...
		autostart(target, room, user) {
			...
			if (toID(target) === 'off') {
				...
				this.addModAction(`${user.name} has turned off the SGS autostart timer.`);
			...
			if (game.state !== 'signups') throw new Chat.ErrorMessage("The game of SGS has already started.");
			if (game.autostartTimer) clearTimeout(game.autostartTimer);
			game.autostartTimer = setTimeout(() => {
				game.onStart(true);
			}, amount * 1000);
			this.addModAction(`${user.name} has set the SGS autostart timer to ${amount} seconds.`);
		},
		...
		disqualify(target, room, user) {
			...
			this.privateModAction(`${user.name} has disqualified ${disqualified} from the SGS game.`);
			this.modlog('SGS DQ', toID(target));
			room.add(`${disqualified} has been disqualified from the SGS game.`).update();
		},
		...
		join(target, room, user) {
			...
			return this.sendReply("You have joined the game of SGS.");
		},
		...
		leave(target, room, user) {
			...
			return this.sendReply("You have left the game of SGS.");
		},

		play(target, room, user) {
			const game = this.requireGame(SGS);
			if (!game) throw new Chat.ErrorMessage("There is no SGS game going on in this room right now.");
			const player: SGSPlayer | undefined = game.playerTable[user.id];
			if (!player) throw new Chat.ErrorMessage(`You are not in the game of SGS.`);
		...
		suppress(target, room, user) {
			...
			if (state === undefined) {
				return this.sendReply(`Suppression of SGS game messages is currently ${game.suppressMessages ? 'on' : 'off'}.`);
			}
			if (state === game.suppressMessages) {
				throw new Chat.ErrorMessage(`Suppression of SGS game messages is already ${game.suppressMessages ? 'on' : 'off'}.`);
			}

			game.suppressMessages = state;

			this.addModAction(`${user.name} has turned ${state ? 'on' : 'off'} suppression of SGS game messages.`);
			this.modlog('SGS SUPRESS', null, (state ? 'ON' : 'OFF'));
		},
		...
		spectate(target, room, user) {
			...
			if (!game.suppressMessages) throw new Chat.ErrorMessage("The current SGS game is not suppressing messages.");
			if (user.id in game.spectators) throw new Chat.ErrorMessage("You are already spectating this game.");

			game.spectators[user.id] = 1;
			this.sendReply("You are now spectating this private SGS game.");
		},
		...
		unspectate(target, room, user) {
			...
			this.sendReply("You are no longer spectating this private SGS game.");
		},
	},

	sgshelp: [
		`/uno create [player cap] - creates a new SGS game with an optional player cap (default player cap at 12). Use the command [createpublic] to force a public game or [createprivate] to force a private game. Requires: % @ # &`,
		`/uno setcap [player cap] - adjusts the player cap of the current SGS game. Requires: % @ # &`,
		...
		`/uno end - ends the current game of SGS. Requires: % @ # &`,
		`/uno start - starts the current game of SGS. Requires: % @ # &`,
		...
		`/uno [spectate|unspectate] - spectate / unspectate the current private SGS game.`,
