...
export const commands: Chat.ChatCommands = {
	...
	saveserverreplay(target, room) {
		...
		const isFFA = room.battle.gameType === 'freeforall';
		const p1 = isFFA ? 'p1' : room.p1?.name;
		const p2 = isFFA ? 'p2' : room.p2?.name;
		...
		if (isFFA) link = `${room.roomid.slice(7)}.html`;
		let rating = 0;
		...
		out.on('open', () => {
			out.write(
				`<!DOCTYPE html>\n` +
				`<meta charset="utf-8" />\n` +
				`<title>${format} replay: ${p1} vs. ${p2}</title>\n` +
				`<meta name="description" content="Watch a replay of a Pokémon battle between ${p1} and ${p2} (${format})" />` +
				'<style>\n' +
				...
				`<h1 style="font-weight:normal;text-align:left"><strong>${format}</strong>: <a href="https://pokemonshowdown.com/users/${toID(p1)}" class="subtle" target="_blank">${p1}</a> vs. <a href="https://pokemonshowdown.com/users/${toID(p2)}" class="subtle" target="_blank">${p2}</a></h1>\n` +
