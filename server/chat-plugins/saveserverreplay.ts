	...
	saveserverreplay(target, room) {
		...
		out.on('open', () => {
			out.write(
				...
				`<meta charset="utf-8" />\n` +
				'<!-- version 1 -->\n' + 
				`<title>${room.battle?.format} replay: ${room.p1} vs. ${room.p2}</title>\n` + 
				'<style>\n' + 
				...
				`<h1 style="font-weight:normal;text-align:center"><strong>${room.battle?.format}</strong><br /><a href="http://pokemonshowdown.com/users/${toID(room.p1?.name)}" class="subtle" target="_blank">${room.p1?.name}</a> vs. <a href="http://pokemonshowdown.com/users/${toID(room.p2?.name)}" class="subtle" target="_blank">${room.p2?.name}</a></h1>\n` + 				
				`<script type="text/plain" class="battle-log-data">${battleLog}</script>\n` + 
				'</div>\n' + 
				// @ts-ignore
				'<div class="battle-log battle-log-inline"><div class="inner">' + battle.scene.log.elem.innerHTML + '</div></div>\n' + 
				'</div>\n' + 
				'<script>\n' + 
